...
import 'package:app_core/ui/leads/krep_lead_form.dart';
...
class KREPListing extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _KREPListingState();
}

class _KREPListingState extends State<KREPListing> {
  List<KLead>? leads;
  ...
  void toCheckInCreate() async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (ctx) => KREPLeadForm(lead: null),
      ),
    );
    loadLeads();
  }
  ...
  void toCheckInEdit([KLead? lead]) async {
    if (lead != null)
      await Navigator.of(context)
          .push(MaterialPageRoute(builder: (ctx) => KREPLeadForm(lead: lead)));
    loadLeads();
  }
    ...
    final body = this.leads == null
        ...
        : this.leads!.length == 0
            ...
            : ListView.separated(
                ...
                separatorBuilder: (_, __) => KDivider(margin: EdgeInsets.zero),
              );
...
class _CheckInItem extends StatelessWidget {
  ...
  String prettyGrade(String grade) {
    if (grade == "-1") {
  ...
  Widget build(BuildContext context) {
    final info = Column(
      ...
      children: <Widget>[
        ...
        if (KStringHelper.isExist(lead.userType) ||
            KStringHelper.isExist(lead.gigMedium) ||
            KStringHelper.isExist(lead.grade)) ...[
          SizedBox(height: 4),
          KIconLabel(
            icon: Icons.info_outline,
            textOverflow: TextOverflow.ellipsis,
            text:
                "${lead.userType ?? ""}${KStringHelper.isExist(lead.gigMedium) ? "${KStringHelper.isExist(lead.userType) ? " - " : ""}${lead.gigMedium}" : ""}${KStringHelper.isExist(lead.grade) ? "${KStringHelper.isExist(lead.userType) || KStringHelper.isExist(lead.gigMedium) ? " - " : ""}${prettyGrade(lead.grade!)}" : ""}",
            textStyle: Theme.of(context).textTheme.bodyText1!,
            isFlexible: true,
          ),
        ],
        if (lead.leadNote != null) ...[
          SizedBox(height: 4),
          KIconLabel(
