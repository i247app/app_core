...
class KPeerVoipCall extends StatefulWidget {
  final String? refApp;
  final String? refID;
  ...
  const KPeerVoipCall({
    Key? key,
    this.refApp = KWebRTCConference.APP_CHAT,
    this.refID,
    this.conferenceSlug,
    this.conferencePass,
  }) : super(key: key);
  ...
  @override
  State<KPeerVoipCall> createState() => _KPeerVoipCallState();
}

class _KPeerVoipCallState extends State<KPeerVoipCall> {
  ...
  KWebRTCMember? get currentMeetingMember =>
      currentMeeting?.webRTCMembers?.firstWhereOrNull((webRTCMember) =>
      ...
      try {
        await setupCall(
          KWebRTCConference()
            ...
            ..conferencePass = widget.conferencePass
            ..refApp = widget.refApp
            ..refID = widget.refID,
        );
      } catch (ex) {}
  ...
  Future _onPopInvoked(bool didPop) async {
    ...
    if (currentMeeting == null) return safePop(true);
