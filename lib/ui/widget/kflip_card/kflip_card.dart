...
import 'kflip_card_transition.dart';
import 'kflip_card_controller.dart';

enum KCardSide {
  front,
  back;

  /// Return [KCardSide] associated with the [AnimationStatus]
  factory KCardSide.fromAnimationStatus(AnimationStatus status) {
    switch (status) {
      case AnimationStatus.dismissed:
      case AnimationStatus.reverse:
        return KCardSide.front;
      case AnimationStatus.forward:
      case AnimationStatus.completed:
        return KCardSide.back;
    }
  }

  /// Return the opposite of the this [KCardSide]
  KCardSide get opposite {
    switch (this) {
      case KCardSide.front:
        return KCardSide.back;
      case KCardSide.back:
        return KCardSide.front;
    }
...
/// )
/// ```
class KFlipCard extends StatefulWidget {
  const KFlipCard({
    super.key,
    required this.front,
    required this.back,
    ...
    this.initialSide = KCardSide.front,
    this.autoFlipDuration,
  });

  /// The initially shown side of the card
  final KCardSide initialSide;
  ...
  /// Assign a controller to the [KFlipCard] to control it programmatically
  ///
  /// {@macro flip_card_controller.example}
  final KFlipCardController? controller;
  ...
  /// with the final [KCardSide]
  final void Function(KCardSide side)? onFlipDone;

  /// The [Duration] a turn animation will take.
  final Duration duration;

  @override
  State<StatefulWidget> createState() => KFlipCardState();
}

/// State associated with a [KFlipCard] widget.
///
/// A [KFlipCardState] can be used to [flip], [flipWithoutAnimation], [skew]
/// or [hint] the associated [KFlipCard]
class KFlipCardState extends State<KFlipCard>
    with SingleTickerProviderStateMixin {
  late AnimationController controller;

  @override
  void didUpdateWidget(KFlipCard oldWidget) {
  ...
  void initState() {
    ...
    controller = AnimationController(
      value: widget.initialSide == KCardSide.front ? 0.0 : 1.0,
  ...
  KCardSide getOppositeSide() {
    return KCardSide.fromAnimationStatus(controller.status).opposite;
  }
  ...
  Widget build(BuildContext context) {
    final child = KFlipCardTransition(
      front: widget.front,
      back: widget.back,
